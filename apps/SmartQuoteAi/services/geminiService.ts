...
export const analyzeJobEstimate = async (
  ...
  try {
    ...
    const responseSchema = {
      ...
      properties: {
        summary: { type: SchemaType.STRING, description: "Executive summary of the scope, methods, and complexity factors identified." },
        items: {
          type: SchemaType.ARRAY,
          items: {
            type: SchemaType.OBJECT,
            properties: {
              category: { type: SchemaType.STRING, enum: ["Material", "Labor", "Equipment", "Permits", "Other"] },
              name: { type: SchemaType.STRING, description: "Detailed name of the item or task (e.g., '1/2 inch Drywall Sheets')" },
              quantity: { type: SchemaType.NUMBER, description: "Calculated quantity including waste factor" },
              unit: { type: SchemaType.STRING, description: "Unit (sqft, hrs, ea, lbs, gal)" },
              reasoning: { type: SchemaType.STRING, description: "Explanation of math (e.g., 'Wall area 400sf / 32sf per sheet + 15% waste')" },
              tiers: {
                type: SchemaType.OBJECT,
                properties: {
                  budget: {
                    ...
                    properties: {
                      ...
                      description: { type: SchemaType.STRING, description: "Specific product or method for budget tier" },
                      products: {
                        type: SchemaType.ARRAY,
                        items: {
                           type: SchemaType.OBJECT,
                           properties: {
                             ...
                             isBulk: { type: SchemaType.BOOLEAN },
                             bulkQuantity: { type: SchemaType.NUMBER },
                             bulkPrice: { type: SchemaType.NUMBER }
                           }
                        }
                      }
                    }
                  ...
                  standard: {
                    ...
                    properties: {
                      ...
                      unitPrice: { type: SchemaType.NUMBER },
                      total: { type: SchemaType.NUMBER },
                      description: { type: SchemaType.STRING, description: "Industry standard material/labor rate" },
                      products: {
                        type: SchemaType.ARRAY,
                         items: {
                           type: SchemaType.OBJECT,
                  ...
                  premium: {
                    ...
                    properties: {
                      ...
                      unitPrice: { type: SchemaType.NUMBER },
                      total: { type: SchemaType.NUMBER },
                      description: { type: SchemaType.STRING, description: "High-end material or expert labor rate" },
                      products: {
                        type: SchemaType.ARRAY,
                         items: {
                           type: SchemaType.OBJECT,
    ...
    return JSON.parse(responseText || '{}') as ProjectEstimate;
  } catch (error) {
...
export const parseBusinessCard = async (imageBase64: string): Promise<ClientDetails | null> => {
  try {
    const responseSchema = {
       type: SchemaType.OBJECT,
       properties: {
          name: { type: SchemaType.STRING, description: "Full Name in Title Case" },
          companyName: { type: SchemaType.STRING, description: "Company Name" },
          email: { type: SchemaType.STRING },
          phone: { type: SchemaType.STRING, description: "Formatted phone number" },
          address: { type: SchemaType.STRING, description: "Formatted multi-line address" },
          notes: { type: SchemaType.STRING, description: "Any extra info like title or website" }
       }
    };
    ...
    return JSON.parse(result.response.text() || '{}') as ClientDetails;
  } catch (e) {
  ...
  try {
    const responseSchema = {
      ...
      properties: {
        ...
        category: { type: SchemaType.STRING, enum: ["Material", "Labor", "Subcontractor", "Other"] },
        items: {
          type: SchemaType.ARRAY,
          items: {
            type: SchemaType.OBJECT,
    ...
    const result = await model.generateContent({
      contents: [{ role: "user", parts: [
        { inlineData: { mimeType: 'image/jpeg', data: base64Data } }, 
        { text: "Extract merchant, date, and line items." }
      ]}]
    });
...
export const generatePaymentReminder = async (clientName: string, invoiceNum: string, amount: string, daysOverdue: number): Promise<string> => {
  try {
    ...
    const result = await model.generateContent(prompt);
    return result.response.text() || "Please remit payment.";
...
export const summarizeDocument = async (text: string): Promise<string> => {
  try {
     ...
     const result = await model.generateContent(prompt);
     return result.response.text() || "Could not summarize.";
