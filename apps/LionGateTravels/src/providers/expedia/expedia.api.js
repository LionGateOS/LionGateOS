...
let config = null;
export function configureExpedia(cfg) {
    config = cfg;
}
/**
 ...
 */
export async function getHotelDetails(propertyId, params) {
    try {
        const queryParams = new URLSearchParams({
            checkin: params.checkIn,
            checkout: params.checkOut,
 ...
 */
function transformExpediaHotels(data) {
    if (!data?.properties)
        return [];
    return data.properties.map((prop) => transformExpediaHotel(prop));
}
function transformExpediaHotel(prop) {
    ...
    return {
        ...
        rooms: prop.rooms?.map((r) => ({
            ...
            beds: r.bed_groups?.map((bg) => bg.description) || [],
            price: {
 ...
 */
function transformExpediaCars(data) {
    if (!data?.vehicles)
        return [];
 ...
 */
function transformExpediaActivities(data) {
    if (!data?.activities)
        return [];
    return data.activities.map((activity) => ({
        ...
        images: activity.images?.map((img) => img.url) || [],
 ...
 */
function generateExpediaAffiliateUrl(type, id) {
